...
<head>
    ...
    <style>
        ...
        body { font-family: 'Helvetica Neue', Arial, sans-serif; background-color: #eef2f5; color: #333; margin: 0; padding: 20px; }
        .main-container { max-width: 600px; margin: 0 auto; background: white; padding: 30px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); }
...
def submit_registration():
    data = {
        ...
        "course": {"1": "3D列印基礎認識實務課程", "2": "3D列印後處理進階課程"}.get(request.form.get('course_id')),
    ...
    payload = {
        "embeds": [{
            ...
            "color": 1104926,
            "fields": [
                {"name": "👤 學員姓名", "value": data["name"], "inline": True},
                {"name": "👥 報名人數", "value": data["people"], "inline": True},
                {"name": "📚 課程項目", "value": data["course"], "inline": False},
                {"name": "📞 聯絡電話", "value": data["phone"], "inline": True},
                {"name": "✉️ 電子信箱", "value": data["email"], "inline": False}
            ],
            "footer": {"text": "系統即時轉寄"}
        }]
    }
