...
<head>
    ...
    <style>
        :root { --primary-color: #1a4985; --secondary-color: #007bff; --accent-color: #f59f00; --dark-color: #2b3a4a; --light-bg: #f8f9fa; }
        ...
        .avatar-title { font-size: 10px; opacity: 0.8; font-weight: bold; letter-spacing: 1px; margin-bottom: -2px; }
        .avatar-main { font-size: 30px; font-weight: bold; color: var(--accent-color); text-shadow: 1px 1px 3px rgba(0,0,0,0.5); }
        .avatar-sub { font-size: 9px; background: var(--accent-color); color: #2b3a4a; padding: 1px 4px; border-radius: 3px; font-weight: bold; scale: 0.9; }
        ...
        button { width: 100%; background: linear-gradient(135deg, #007bff, #0056b3); color: white; border: none; padding: 15px; font-size: 18px; font-weight: bold; border-radius: 6px; cursor: pointer; margin-top: 10px; }
...
def submit_registration():
    ...
    selected_course = {"1": "3D列印基礎認識實務課程", "2": "3D列印後處理進階課程"}.get(course_id, "未知課程")
    ...
    payload = {
        "embeds": [{
            ...
            "fields": [
                ...
                {"name": "👥 報名人數", "value": str(people), "inline": True},
    ...
    return f"""<script>alert('【唯修科技】{name} 同學，報名已提交！'); window.location.href = '/';</script>"""
