...
# 雲端防錯版 HTML 字串（已將 JS 正則表達式進行跳脫，防止傳輸字元漏失）
HTML_CONTENT = """
<!DOCTYPE html>
<html lang="zh-TW">
<head>
...
<body class="bg-slate-50 text-slate-800 font-sans">
                    ...
                    <table class="w-full text-left border-collapse text-sm">
                        <thead>
                            <tr class="bg-blue-900 text-white border-b border-slate-300">
                                <th class="p-3 font-bold">課程時數</th>
            ...
            <div class="grid md:grid-cols-1 gap-6 max-w-xl mx-auto">
                <div class="bg-white rounded-2xl shadow-md overflow-hidden border border-slate-200 p-6 flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6">
                    <div class="w-32 h-32 rounded-xl overflow-hidden shadow-inner bg-slate-100 flex-shrink-0 border-2 border-blue-900">
                        <img src="/static/IMG_8743.JPG" alt="Sebastain 講師" class="w-full h-full object-cover">
                    </div>
            ...
            <div class="bg-white p-8 rounded-2xl shadow-lg border border-slate-200">
                ...
                <form id="regForm" onsubmit="handleFormSubmit(event)" novalidate>
                    ...
                    <div class="mb-6">
                        ...
                        <p id="emailError" class="text-xs text-red-600 mt-1 hidden">⚠️ 電子郵件格式有誤。</p>
                    </div>
    ...
    <script>
        function handleFormSubmit(event) {
            ...
            let hasError = false;
            if (!phoneRegex.test(phoneInput)) { phoneError.classList.remove('hidden'); hasError = true; } else { phoneError.classList.add('hidden'); }
            if (!emailRegex.test(emailInput)) { emailError.classList.remove('hidden'); hasError = true; } else { emailError.classList.add('hidden'); }
            if (hasError) return;

            const formData = new FormData(form);
            fetch('/submit_registration', { method: 'POST', body: formData })
